...
const webhookWriter = (config: any) => {
  ...
  console.log("shopify.user.webhooks", shopify.user.webhooks);
  for (const entry in shopify.user.webhooks) {
    const subscription = {
      ...
      uri: shopify.user.webhooks[entry].url.startsWith("/api/webhooks/")
        ? `${process.env.NEXT_PUBLIC_SHOPIFY_APP_URL}${shopify.user.webhooks[entry].url}`
...
async function writeToApi() {
  try {
    ...
    console.log("webhookImports", webhookImports);
